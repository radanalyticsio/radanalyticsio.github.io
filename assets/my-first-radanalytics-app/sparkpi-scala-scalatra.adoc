...
Now it is time to implement the actual Spark job that will calculate π. For this example, the code is adapted directly from the https://github.com/apache/spark/blob/master/examples/src/main/scala/org/apache/spark/examples/SparkPi.scala[SparkPi Example]. The job will be encapsulated inside of an object that exposes only one method `calculate()`. Create a new Scala class, `src/main/resources/io/radanalytics/examples/scalatra/sparkpi/SparkPi.scala` and implement `calculate()` the method as shown below:
...
This deployment can be tested loby running `java -jar target/scala-2.11/tutorial-sparkpi-scala-scalatra-assembly-0.0.1-SNAPSHOT.jar. The service should start, but will fail when servicing requests, unless you have a Spark cluster running locally.
...
Now that the service is up and running, you can return to the https://radanalytics.io/my-first-radanalytics-app.html#user[My First Radanalytics Application page] to learn how to interact with this new microservice.
